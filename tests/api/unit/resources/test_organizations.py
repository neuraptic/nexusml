...
class TestUser:
    """
    Classes inside this class are used to group function tests for better organization and readability.
    """

    class TestDownloadAccountData:

        @pytest.fixture(autouse=True)
        def mock_env(self, class_mocker):
            class_mocker.patch.dict(
                os.environ, {
                    'AUTH0_NEXUSML_ACTIONS_CLIENT_ID': 'test_client_id',
                    'AUTH0_NEXUSML_ACTIONS_CLIENT_SECRET': 'test_client_secret',
                    'AUTH0_DOMAIN': 'test_domain'
                })

        @pytest.fixture
        def mock_get_auth0_user_data(self, mocker):
            return mocker.patch('nexusml.api.resources.organizations.get_auth0_user_data')

        @pytest.fixture
        def mock_get_auth0_management_api_token(self, mocker):
